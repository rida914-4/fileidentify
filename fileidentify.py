...
def folder_handler():
    # Handle the folder given in argument here
    folder_user = arg_parser()
    if folder_user[0] in ['c', 'd', 'e', 'f', 'C', 'D']:
...
def main():
    ...
    for path, folders, files in os.walk(folder):
        ...
        for file_name in files:
            ...
            mime_result = mime_finder(file_full)
            ...
            try:
                ...
                exception_list.append(file_full)
                continue
            ...
            if hex_content and filetype.has_key(hex_content[0:2].upper()):
                for key, value in filetype[hex_content[0:2].upper()].iteritems():
                    if key in hex_content[0:7].upper() or hex_content[0:7].upper() in key and file_num == 0:
                        file_num = 1
                        chunk.append(file_full + ' : ' + value + str(mime_result))
            else:
                file_num = 1
                chunk.append(file_full + ' : Not in the signature database' + str(mime_result))
